...
impl SelectionSortVisualizer {
    ...
    fn draw(&mut self, stdout: &mut std::io::Stdout) {
        ...
        // --- Title ---
        let title = "TOGISOFT SELECTION SORT VISUALIZER";
    ...
    fn draw_array(&self, stdout: &mut std::io::Stdout, width: u16, height: u16) {
        ...
        for (i, &value) in self.array.iter().enumerate() {
            ...
            let (fg_color, bg_color) = match self.states[i] {
                ...
                SelectionState::CurrentMin => (Color::Yellow, Color::DarkYellow), // Current minimum element
                SelectionState::Comparing => (Color::Magenta, Color::DarkMagenta), // Being compared
                SelectionState::Selected => (Color::White, Color::DarkBlue), // Position being filled
                SelectionState::Swapping => (Color::Red, Color::DarkRed), // Being swapped
            ...
            let operation = match self.phase {
                ...
                SelectionPhase::SearchingMin => {
                    format!("Step {}/{}: Searching for minimum in range [{}..{}] - Comparing {} with current min {} at index {}",
                            ...
                            self.array[self.current_j], self.array[self.min_index], self.min_index)
                },
                SelectionPhase::FoundMin => {
                    format!("Step {}/{}: Found minimum {} at index {} - Ready to swap with position {}",
                            self.current_i + 1, self.array.len(),
                            self.array[self.min_index], self.min_index, self.current_i)
                },
                SelectionPhase::Swapping => {
                    format!("Step {}/{}: Swapping {} (pos {}) with {} (pos {})",
                            self.current_i + 1, self.array.len(),
                            self.array[self.current_i], self.current_i,
                            self.array[self.min_index], self.min_index)
                },
            };
    ...
    fn step(&mut self) -> bool {
        if self.completed || self.current_i >= self.array.len() {
            return false;
        }
